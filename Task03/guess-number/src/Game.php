...
class Game
{
    public function playInteractive(): array
    {
        ...
        for ($i = 1; $i <= $maxAttempts; $i++) {
            $input = $this->promptNonEmpty(sprintf('Попытка %d/%d — введите число (1..%d)', $i, $maxAttempts, $maxNumber));
            
            if (!is_numeric($input)) {
                $this->writeln("Введите корректное число!");
                $i--; // не засчитываем попытку
                continue;
            }
    ...
    private function promptNonEmpty(string $message): string
    {
        ...
        while (trim($value) === '') {
            ...
            if ($value === '') $this->writeln('Поле не может быть пустым.');
        }
        return $value;
    }
